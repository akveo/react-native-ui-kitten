...
export class EvaStyleSheet {
  ...
  static createThemedStyle = (style: StyleType, theme: ThemeType): StyleType => {
    return Object.keys(style).reduce((acc: StyleType, key: string): StyleType => {
      ...
      return { ...acc, [key]: EvaStyleSheet.getThemeValue(value, theme, value) };
    }, {});
  };

  static createCompiledTheme = (theme: ThemeType): ThemeType => {
    return Object.keys(theme).reduce((acc: ThemeType, key: string): ThemeType => {
      return { ...acc, [key]: EvaStyleSheet.getThemeValue(key, theme, key) };
    }, {});
  };

  static getThemeValue = (name: string, theme: ThemeType, fallback?: ThemeValue): ThemeValue | undefined => {
    if (EvaStyleSheet.isReference(name)) {
      const themeKey: string = EvaStyleSheet.toThemeKey(name);
      return EvaStyleSheet.findThemeValue(themeKey, theme) || fallback;
    }

    return EvaStyleSheet.findThemeValue(name, theme) || fallback;
  };

  static findThemeValue = (name: string, theme: ThemeType): ThemeValue | undefined => {
    const value: ThemeValue = theme[name];

    if (EvaStyleSheet.isReference(value)) {
      const themeKey: string = EvaStyleSheet.toThemeKey(value);
      return EvaStyleSheet.findThemeValue(themeKey, theme);
    }
