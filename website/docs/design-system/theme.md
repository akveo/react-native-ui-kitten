...
sidebar_label: Design System Theme
...
In Eva Design System a **theme** is a set of semantic variables and connections between them, that represents the application's look & feel to achieve the following goals:
...
All available color within the theme. 5 semantic colors (`primary`, `success`, `info`, `warning`, `danger`) and `basic` color (backgrounds and texts). Each color has a pallet of 9 shades, except for `basic`, which has 11 shades. These colors are mostly used by `status` variants of the components.
...
Each style describes text `fontSize`, `fontWeight` and `lineHeight`. For instance, there is a final style applied by `c1` (caption 1) `Text` component:
