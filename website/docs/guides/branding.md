...
There are 6 shades of transparency in Eva. Pick color-basic-600 value and [transform it to rgba format](http://hex2rgba.devoth.com) with adding an alpha channel. In Eva, we start with 8% transparency and move up with increasing it on the same value.
...
![image](/img/articles/guides/branding-overview-backgrounds.png)

---

## Text and Icons

As well as Backgrounds, Text and Icon colors are controlled by Basic shades. In a dark theme, text colors utilize the upper part of the shades (whitish colors) and backgrounds in the lower part.

Sometimes it is required to make Text lighter or darker without modifying Basic palette in order not to break backgrounds. This can be done by configuring extra text variables. Let's take a look at the most useful of them:
...
![image](/img/articles/guides/branding-overview-text.png)

---

## Typography

If your product relies on custom fonts, start with copying `.ttf` file and paste it into `assets` directory. Eva is designed with [Open Sans](https://fonts.google.com/specimen/Open+Sans).

In React Native, fonts can be configured with React Native CLI. Create a `react-native.config.js` file and paste the following code:
...
Run `npx react-native-asset` from the project root to link fonts with the native side. The `react-native link` command was removed from the React Native CLI in 0.69 and no longer exists. When it's done, create a `mapping.json` file and paste the code below to apply fonts to UI Kitten components.
...
In the previous step, we made a single font be used globally. In case we need to configure it more accurately, we may use Text Categories. A single text category controls `fontSize`, `fontWeight` and `fontFamily`.
...
These colors define the rules of how another theme variables are used and should never be used directly in code. Transparent palette is always a `-500` color with an alpha channel. It is needed for disabled, outlined and ghost controls.
...
There is a special color within the semantic group - Basic. It has 11 shades. Also, its transparent palette is always a `-600` color with alpha channel. Be careful when working with basic, as it is used by [both backgrounds and text](#text-and-icons).
...
For instance, default background level is 1 (`background-basic-color-1`). And that's why Input looks well on it by using level 2.

---

## Conclusion

In this guide, you learned how to configure custom theme with Eva Design System.

By moving to the [next guide](/docs/guides/icon-packages), you will learn how to use icon packages in React Native.
