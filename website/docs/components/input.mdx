...
| `caption` | `string \| number \| (props) => ReactElement` | `undefined` | Caption displayed below the input (e.g., validation message). |
...
Input also accepts all standard React Native `TextInput` props, including `placeholder`.

Input exposes an imperative API through a ref typed as `InputRef`, with `focus()`, `blur()`, `isFocused()` and `clear()`.
