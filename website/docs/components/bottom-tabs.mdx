...
| `selected` | `boolean` | `false` | Whether the tab is rendered as selected. Managed by BottomNavigation. |
| `onSelect` | `(selected: boolean) => void` | `undefined` | Called when the tab is selected. Managed by BottomNavigation. |
| `appearance` | `'default'` | `'default'` | Appearance defined in the Eva mapping. |

BottomNavigationTab also accepts all standard React Native `TouchableOpacity` props.
