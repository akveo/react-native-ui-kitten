...
| `renderIcon` | `(props) => ReactElement` | `undefined` | Render function for an icon displayed inside the circle instead of the progress text. |
| `textStyle` | `StyleProp<TextStyle>` | `undefined` | Additional styles applied to the progress text. |
| `iconStyle` | `StyleProp<ImageStyle>` | `undefined` | Additional styles applied to the icon returned by `renderIcon`. |
| `animationConfig` | `Partial<CircularProgressBarAnimationConfig>` | `undefined` | Animation configuration — duration and easing. |
