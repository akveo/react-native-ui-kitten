...
To achieve this and allow UI Kitten to grow faster and better, we had to refactor a lot of the internal implementations,as well as public APIs.

We hope that the next UI Kitten versions won't receive such a significant upgrade, 
and that the amount of breaking changes will be kept as low as possible.

### Migration Purposes

- Flexible styling with new convenient and unified component interfaces;
- A <a href="https://en.wikipedia.org/wiki/Declarative_programming" target="_blank">declarative</a> way of rendering nested components;
...
If a function is passed, it will be called with style properties provided by Eva.
This makes *{componentName}Style* properties redundant.

2. In order to follow the rule above, CheckBox, Radio and Toggle components now accept their text as a child element, and no longer as a *text* attribute.

3. The *icon* properties has been renamed to `accessoryLeft` and `accessoryRight`.
...
```bash
npm i @ui-kitten/eva-icons@latest @eva-design/material@latest

// Using Yarn?
yarn add @ui-kitten/eva-icons@latest @eva-design/material@latest
```
...
[Button](components/button) no longer accepts a `textStyle` property.
Instead, if custom styles are required, use a function component with additional text styles as the child element.
...
[Radio](components/radio) no longer accepts a `text` property.
It was moved to a child, in favor of using a declarative way of building components.
...
The `textStyle` property is no longer supported.
...
[CheckBox](components/checkbox) no longer accepts a `text` property.
It was moved to a child, in favor of using a declarative way of building components.
...
[Toggle](components/toggle) no longer accepts a `text` property.
It was moved to a child, in favor of using a declarative way of building components.
...
[Input](components/input) no longer has an `icon` property.
Instead, use the `accessoryRight` or `accessoryLeft` properties.
...
The `labelStyle` and `captionStyle` properties are no longer supported.
Instead, if custom styles are required, use a function component with additional text styles as the child element.
...
[Datepicker](components/datepicker) does not have an `icon` property anymore.
Instead, the `accessoryRight` or `accessoryLeft` properties should be used.
...
The `labelStyle`, `captionStyle` and `placeholderStyle` properties are no longer supported.
Instead, if custom styles are required, use a function component with additional text styles in the corresponding properties.
...
We also strongly recommend visiting the [Select Overview](components/select) page to become familiar with its API
...
The `labelStyle` and `captionStyle` properties are no longer supported.
Instead, if custom styles are required, use a function component with additional text styles, passed to the corresponding property.
...
[Tabs](components/bottom-tabs/overview#bottomnavigationtab) no longer accepts a `titleStyle` property. 
Instead, if custom styles are required, use a function component with additional text styles, passed to the `title` property.
...
[Tabs](components/top-tabs/overview#tab) no longer accepts the `titleStyle` property. 
Instead, if custom styles are required, use a function component with additional text styles, passed to the `title` property.
...
This made us remove the `titleStyle` and `subtitleStyle` properties.
...
replacing the old `leftControl` and `rightControls`.
...
It no longer exports a `DrawerHeaderFooter` component as we find it useless.
Instead, `header` and `footer` properties accept styles provided by Eva that should be used as the style
of the container for the Header or Footer of Cards.
...
We also strongly recommend visiting the [Menu Overview](components/menu) page to become familiar with its API
since its selection functionality was completely reworked.

### Popover

[Popover](components/popover) now accept its content as a child element.
A component relative to which the content is rendered is now passed to `anchor` property.
...
[Tooltip](components/tooltip) now accept its text as a child element.
A component relative to which the content is rendered is now passed to `anchor` property.
...
[OverflowMenu](components/overflow-menu) `data` property is replaced in favor of child elements,
which makes it possible to accept any type of data not requiring you to have `title` properties for its items.
Now its contents are fully controlled by you as a developer and should be passed as child elements.
...
The `titleStyle` and `descriptionStyle` properties are no longer supported.
Instead, if custom styles are required, use a function component with additional text styles, passed to the corresponding property.
...
replacing the old `icon` and `accessory` properties.
...
[Card](components/card) no longer exports a `CardHeader` component as we find it useless.
Instead, `header` and `footer` properties accept styles provided by Eva that should be used as the style
of the container for a Header or Footer of Cards.
